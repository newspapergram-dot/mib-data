...
from fundamentals import get_fundamentals, fundamentals_summary_line
...
def main():
    ...
    # prezzo corrente reale per il cross-check anti-dati-stantii di fundamentals.py
    mib_rows = _load("mib_data.csv")
    price_now_map = {}
    for r in mib_rows:
        t = r.get("ticker")
        if t and r.get("close"):
            price_now_map[t] = r.get("close")  # mib_data.csv e' ordinato per data: l'ultimo vince
    ...
    fundamentals_rows = []  # per l'export data/fundamentals.csv
    ...
    for row in cands:
        ...
        block.append("  " + fundamentals_summary_line(fnd))
        if veto:
...
.fnd{{font-size:12px;color:#555;margin:8px 0 0;font-style:italic}}
...
<p style="font-size:12px;color:#888;margin-top:24px">Generato da weekly_report.py — edge calibrato su regime bull (gen-mag 2026). Sizing prudenziale. Non e' consulenza finanziaria.</p>
    ...
    # --- export data/fundamentals.csv (i fondamentali usati in questo report) ---
